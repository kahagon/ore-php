...
#define PHP_MODE_SCRIPT_SERVER 100
...
static void php_cli_usage(char *argv0)
{
	...
	printf( "Usage: %s [options] [-f] <file> [--] [args...]\n"
				...
				"\n"
				"  --script-server  Run script server\n"
				"\n"
	...
	zend_try {
		...
		while ((c = php_getopt(argc, argv, OPTIONS, &php_optarg, &php_optind, 0, 2)) != -1) {
			...
			case 15:
				...
				break;
                        case 20:
                            printf("behavior = PHP_MODE_SCRIPT_SERVER\n");
                                behavior = PHP_MODE_SCRIPT_SERVER;
                                break;
			default:
				break;
			}
		...
		*arg_excp = arg_free; /* reconstuct argv */
		...
		case PHP_MODE_CLI_DIRECT:
			...
			break;
                case PHP_MODE_SCRIPT_SERVER:
                    cli_register_file_handles(TSRMLS_C);

                    char *port = "18888";
                    do_script_server(port);
                    break;
