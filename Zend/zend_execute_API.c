...
/* }}} */
/*
ZEND_API int zend_eval_string_ex_d(HashTable *ht, char *str, zval *retval_ptr, char *string_name, int handle_exceptions TSRMLS_DC)
{
	...
	Z_TYPE(pv) = IS_STRING;
        ...
        if (zend_hash_exists(ht, Z_STRVAL(pv), Z_STRLEN(pv))) {
            zend_hash_find(ht, Z_STRVAL(pv), Z_STRLEN(pv), (void **)&new_op_array);
        } else {
            original_compiler_options = CG(compiler_options);
            CG(compiler_options) = ZEND_COMPILE_DEFAULT_FOR_EVAL;
            new_op_array = zend_compile_string(&pv, string_name TSRMLS_CC);
            CG(compiler_options) = original_compiler_options;
            zend_hash_add(ht, Z_STRVAL(pv), Z_STRLEN(pv), (void *)new_op_array, sizeof(zend_op_array), NULL);
        }
